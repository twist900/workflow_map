...
public class DAO extends Observable {
        ...
        if( workCRS != null ){
            try {
                ...
                while (workCRS.next()) {
                    ...
                    work.setWorkRelations(this.getAllWorkConnsArray(work.getWorkId()));
                    works.add(work);
                }
            } catch (SQLException e) {
                e.printStackTrace();
    ...
    public ArrayList<WorkRelation> getAllWorkConnsArray(String workId){
        ArrayList<WorkRelation> workRelations = new ArrayList<WorkRelation>();
    ...
    public void insertWorkRelations(ArrayList<WorkRelation> workRelations){
        String insertTableSQL = "INSERT INTO work_relation"
                + "(ID, WORK_ID_PREV, WORK_ID_NEXT) VALUES"
                + "(?,?,?)";
        PreparedStatement preparedStatement = null;
        for(WorkRelation workRelation : workRelations){
            try {
                preparedStatement = conn.prepareStatement(insertTableSQL);
